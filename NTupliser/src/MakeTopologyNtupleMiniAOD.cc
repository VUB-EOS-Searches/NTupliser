...
void MakeTopologyNtupleMiniAOD::fillPV(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    if (pvHandle.isValid()) {
        ...
        for (auto it{pv.begin()}; it != pv.end() && numPVs < numeric_cast<int>(NPVSMAX); it++) {
            ...
            pvY[numPVs] = it->y();
            pvZ[numPVs] = it->z();
            pvCov00[numChsTrackPairs] = it->covariance(0,0);
            pvCov01[numChsTrackPairs] = it->covariance(0,1);
            pvCov02[numChsTrackPairs] = it->covariance(0,2);
            pvCov10[numChsTrackPairs] = it->covariance(1,0);
            pvCov11[numChsTrackPairs] = it->covariance(1,1);
            pvCov12[numChsTrackPairs] = it->covariance(1,2);
            pvCov20[numChsTrackPairs] = it->covariance(2,0);
            pvCov21[numChsTrackPairs] = it->covariance(2,1);
            pvCov22[numChsTrackPairs] = it->covariance(2,2);
